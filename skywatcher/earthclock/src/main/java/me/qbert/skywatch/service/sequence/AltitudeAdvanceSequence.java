...
public class AltitudeAdvanceSequence implements SequenceElementI {
	private CelestialObject celestialObject;
	private ObservationTime observationTime;
	private double altitude;
	
	public AltitudeAdvanceSequence(CelestialObject celestialObject, ObservationTime observationTime, double altitude) {
	...
	public void update() throws UninitializedObject {
		...
		while (advanceTime > 1000L) {
			for (long ts = currentTime;ts < abortTime;ts += advanceTime) {
				...
				double newAlt = celestialObject.getCurrentDirectionAsAltitudeAzimuth().getAltitude();
				double newAltDelta = Math.abs(altitude - newAlt);
