...
public class StereoAzimuthalObjects extends AbstractCelestialObjects {
	private AzimuthalEquidistantNPPObjects leftPanel;
	private AzimuthalEquidistantNPPObjects rightPanel;
	
	private SplitContainerRenderer splitContainer;
	private double stereoAngle = 1.5;
	
	public StereoAzimuthalObjects(Canvas canvas) throws Exception {
		super(canvas);
	}
	...
	protected void postConstructorInit() throws Exception {
		this.leftPanel = new AzimuthalEquidistantNPPObjects(getCanvas());
		this.rightPanel = new AzimuthalEquidistantNPPObjects(getCanvas());
...
//		boolean dstChanged = getSequenceGenerator().advanceSequence(getDstRotateSetting() == 2);
	...
	protected Point2D.Double getMoonShadowCoordinate(MapCenterMode centerMode, GeoLocation subSunPoint, GeoLocation subMoonPoint) {
		return leftPanel.getMoonShadowCoordinate(centerMode, subSunPoint, subMoonPoint);
	}
